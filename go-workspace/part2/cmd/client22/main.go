...
var (
	...
	hosts      stringSlice
	threads    = flag.Int("n-t", 1, "Number of threads to use for each server")
	n_servers  = flag.Int("n-s", 1, "Number of servers to use")
	...
	batch_size = flag.Int("batch-size", 5, "batch size")
	...
	clients []pb.ImgDownloadClient

	// others
	wg      sync.WaitGroup
	stats   [2000]statistics
	task_no atomic.Int32
)

func main() {
	flag.Var(&hosts, "host", "server addresses")
	flag.Parse()
	if 2000%(*batch_size) != 0 {
		log.Printf("batch_size must be a divisor of 2000")
		return
	}

	if len(hosts) != *n_servers {
		log.Printf("Number of servers specified by -n-s is not equal to the number of hosts specified by -host")
		return
	}
	...
	for _, host := range hosts {
		...
		_conn, err := grpc.Dial(host, opts...)
		if err != nil {
			log.Printf("fail to dial: %v", err)
			return // Use return to perform task_no.Unlink() in defer
		}
		defer _conn.Close()
		conn = append(conn, _conn)
		clients = append(clients, pb.NewImgDownloadClient(_conn))
	}

	wg.Add((*threads) * (*n_servers))
	log.Printf("Start downloading images...")
	begin := time.Now()

	for i := 0; i < (*threads)*(*n_servers); i++ {
		client := clients[i%(*n_servers)]
		serverAddr := hosts[i%(*n_servers)]
		go func() {
			defer wg.Done()
			for task := task_no.Add(int32(*batch_size)); task < 2000+int32(*batch_size); task = task_no.Add(int32(*batch_size)) { // May need to modify this
				stream, err := client.GetMultiImgs(context.Background())
				if err != nil {
					log.Printf("Task %d: Error when calling GetMultiImgs: %v", task, err)
					continue
				}

				var start_times map[string]time.Time = make(map[string]time.Time)

				for i := task - int32(*batch_size); i < task; i++ {
					start_times[requests[i]] = time.Now()
					err := stream.Send(&pb.ImgRequest{
						Url: requests[i],
						Sz:  &pb.Size{Width: 128, Height: 128},
					})

					if err != nil {
						log.Printf("Error when sending request %s: %v", requests[i], err)
						continue
					}
				}
				...
				for i := task - int32(*batch_size); i < task; i++ {
					b, err := stream.Recv()
					if err != nil {
						log.Fatalf("client.GetMultiImgs failed: %v", err)
					}
					// Save the image now
					rtt_latency := time.Since(start_times[b.Url]).Nanoseconds()
					filename := filepath.Join(*out_dir, b.Url+".JPEG")
					...
					stats[i] = statistics{
						...
						handle_latency:   b.HandleLatency,
						inserver_latency: b.InserverLatency,
						rtt_latency:      rtt_latency,
						total_latency:    time.Since(start_times[b.Url]).Nanoseconds(),
						ongoing_tasks:    b.OngoingRequests,
					}
				}
			}
		}()
	...
	wg.Wait()
	log.Printf("All images downloaded.")
	total_time := time.Since(begin).Nanoseconds()
	log.Printf("Total time: %f seconds", float64(total_time)/1e9)

	// Save the statistics
	log.Printf("Saving statistics...")
